...
- Stages 1, 3, 5 (1-indexed) have `op1="+"`, `op2="+"`, `op3="<<"`.
...
### 4.3) Inter-round dependencies limit pipelining

The 1110 cycle bound assumes perfect pipelining across all 512 vector groups.
However, rounds are dependent: element `i` in round `h+1` requires the output
from round `h`. Only the 32 vector groups *within* a single round are truly
independent.

Per round: `32 vector groups * 13 valu slots = 416 valu slots`
Minimum cycles per round: `ceil(416 / 6) = 70 cycles`
Total for 16 rounds: `70 * 16 = 1120 cycles`

This suggests the practical valu-limited minimum is closer to **1120 cycles**
rather than 1110, due to pipeline bubbles at round boundaries.
...
With `forest_height=10`, the tree has `2^11 - 1 = 2047` nodes spanning depths
0 to 10 (11 levels total). Within one 16-round loop, walkers descend through
the tree, wrap at the leaves, and continue. The node read happens BEFORE the
wrap, so depths read by round are:

- rounds 0..9: read depths 0..9
- round 10: read depth 10 (leaves), then wrap to root
- rounds 11..15: read depths 0..4

Best-case distinct-node loads per round (capped by batch_size=256):

- d=0..7: `1+2+4+8+16+32+64+128 = 255`
- d=8..10: `min(256,256) + min(512,256) + min(1024,256) = 256+256+256 = 768`
- d=0..4 (after wrap): `1+2+4+8+16 = 31`

Total best-case distinct node loads across all 16 rounds:

`255 + 768 + 31 = 1054` scalar node loads

Load-cycles lower bound for node loads in this ideal reuse scenario:

`ceil(1054 / 2) = 527` cycles

This is *below* the 1120 valu bound, meaning that at the theoretical optimum,
...
These are also below 1120, so they should not be limiting if scheduled well.

Furthermore, if idx/val are kept in scratch between rounds (scratch has 1536
words, batch needs only 512 words for idx+val), we only need:

- Initial vloads: 64 (32 for idx + 32 for val)
- Final vstores: 64
- Load cycles: `ceil(64 / 2) = 32` cycles
- Store cycles: `ceil(64 / 2) = 32` cycles

This optimization makes input/output memory traffic negligible.
...
with additional overhead pushing the achievable optimum above 1120.
...
- Accounting for inter-round dependencies: ~1120 cycles more realistic.
...
is somewhere above 1120, likely in the 1150-1250 range.
